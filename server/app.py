...
# Add middleware to handle CORS headers for all requests
@app.middleware("http")
...
async def options_handler(path: str):
    """Handle OPTIONS preflight requests for all API paths"""
    headers = {
...
@app.options("/api/user-query")
async def options_user_query():
    """Handle OPTIONS preflight request for /api/user-query"""
    headers = {
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
        "Access-Control-Allow-Headers": "Content-Type, Authorization",
...
async def process_authenticated_query(request: UserQueryRequest):
    ...
    try:
        logger.info(f"Received POST request to /api/user-query")
        logger.info(f"Request data: User ID {request.user_id}, Query: {request.query}")
